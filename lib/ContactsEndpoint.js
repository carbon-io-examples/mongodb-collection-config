...
module.exports = o({
  ...
  preInsertObject: function(object, options) {
    object.user = options.user // Set the contacts user field to the authenticated user id
  },
  ...
  preFind: function(options) {
    if (_.isEqual(options.query, {})) {
      options.query = {user: options.user}
    } else {
      options.query = {
        $or: [
          {firstName: options.query}, // We could get fancier and use regex searches
